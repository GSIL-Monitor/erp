 ...
 */
@Component
public class PurchaseReturnSubmitAfter extends IFsmHandler<PurchaseReturned> {
   ...
   /* @Resource
    ...
    private final Logger logger = LoggerFactory.getLogger(PurchaseReturnSubmitAfter.class);
    ...
    public void execute(PurchaseReturned purchaseReturned, EventModel event) {
        ...
        if (ReturnedTypeEnum.INTRANSIT.name().equals(purchaseReturned.getType())) {// 在途退货
            ...
            purchaseReturnedService.changeReturnLockStock(purchaseReturned, purchaseReturnedItems, PurchaseHandleEnum.purchase_cancel.name());
