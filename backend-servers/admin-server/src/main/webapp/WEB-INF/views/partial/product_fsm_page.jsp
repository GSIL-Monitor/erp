    ...
    <control-table :app='app' tablekey='fsm' :loading='app.controls.fsm.loading' :sub='true'>
        ...
        <el-table-column label='源状态' prop='srcState'>
            <template scope='inner'>
                {{inner.row.srcStateDisplay || app.controls.app.product_StateNames[inner.row.srcState] || "(" +
                inner.row.srcState + ")"}}
            </template>
        </el-table-column>
        <el-table-column label='事件' prop='eventName'>
            <template scope='inner'>
                {{inner.row.eventNameDisplay || app.controls.app.product_EventNames[inner.row.eventName] || "(" +
                inner.row.eventName + ")"}}
            </template>
        </el-table-column>
        <el-table-column label='目标状态' prop='dstState'>
            <template scope='inner'>
                {{inner.row.dstStateDisplay || app.controls.app.product_StateNames[inner.row.dstState] || "(" +
                inner.row.dstState + ")"}}
            </template>
        </el-table-column>
