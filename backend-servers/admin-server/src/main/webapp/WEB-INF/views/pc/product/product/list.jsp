...
<body>
    ...
    <template scope='app'>
    <control-table :app='app' :style="`width: ${app.window.innerWidth - 250}  px;`" tablekey='product_table' :loading='app.controls.product_table.loading'>
    <template scope='app' slot='controls'>
        <el-row>
            ...
            <el-col :span='6' style='padding: 5px 15px;'>
                    <control-item type='input' :local='app.local.searchParams' datakey='innerName' label='内部名'></control-item>
                </el-col>
        </el-row>
        <el-row>
            <el-col :span='6' style='padding: 5px 15px;'>
            ...
            <el-col :span='6' style='padding: 5px 15px;'>
               <el-button type='primary' icon='search' style='margin-top: 2px;' @click='app.controls.product_table.searchClick()' :loading='app.controls.product_table.loading'>搜索</el-button>
    ...
    <el-table-column label='操作' width='100'>
        <template scope='inner'>
            ...
            <el-button size='mini' :disabled='!app.controls.app.Perm.revoke' style="margin-left: 0px;" v-if="inner.row.stateEnum=='archiving'" @click='app.controls.product_table.revokeProductThing(inner.row)' >撤回</el-button>
            <el-dialog
                    title="提示"
        ...
        <el-table-column label="图片" width="150">
            <template scope='inner'>
                <el-col style='width: 64px; margin-right: 16px;'><img
                        :src='window.uploadPrefix + "/" + inner.row.mainImageUrl' style='width: 120px; height: 140px;'>
                </el-col>
            </template>
        </el-table-column>
    ...
    <el-table-column label='状态' width='100'>
            <template scope='inner'>
                ...
                <el-row class='breakspace'><a href='javascript: void(0);' @click='window.app.controls.product_table.showFSM(inner.row)'>{{inner.row.stateTime}}</a></el-row>
            </template>
    </el-table-column>
        ...
        <%--<el-table-column label='在售地区'>
                ...
                <!--<el-tabs v-if='inner.row.zoneSet'>-->
                        <!--<el-tab-pane :key='r.zoneId' :name='r.zoneId'>-->
                            <div  v-for='r in inner.row.zoneSet||[]' style='white-space: nowrap;'>{{r.departmentName}}  <span slot='label' style='font-weight: bold; font-size: 12px;'>{{r.zoneName}}</span>{{r.zoneState}} 库存:{{ r.stock}} 最后下单: {{r.lastOrderAt || '暂无'}}</div>
                        <!--</el-tab-pane>-->
                    <!--</el-tabs>-->
...
-->
     ...
     <dialog-create :title="`产品详情`":visible='app.controls.product_table.dialog.detail' @update:visible='app.controls.product_table.dialog.detail=arguments[0]' v-if='app.controls.product_table.selected' v-for='item in [app.controls.product_table.selected]'>
         <el-card>
            <el-row style='font-size: 12px; line-height: 24px;'>
                ...
                <el-col :span='14'>
                    ...
                    <control-item :app='app' :local='item' datakey='innerName' type='edit' @saved='app.controls.product_table.update(item, arguments[0])'></control-item>
                    标题: {{item.title}}<br>
                    品类: <span v-html="item.name || '暂无'"></span><br>
                    特性: <el-tag v-if='item.classifyEnumName'>{{item.classifyEnumName}}</el-tag><span v-if='!item.classifyEnumName'>无</span>
                    <%--<span v-if='!item.sourceURL && !item.purchaseURL'>来源: 暂无</span><a :href='window.app.normalizeURL(item.sourceURL)'>{{item.sourceEnumName}}</a> <a :href='window.app.normalizeURL(item.purchaseURL)' v-if='item.purchaseURL'>采购链接</a><br>--%>
    ...
    <script>
        window.layoutData.controls.product_table = {
            ...
            get(){
                ...
                this.loading = true;
                app.ajax.product.query(parameters).then(result => {
            ...
            showDetail(row){
                window.top.app.open('/product/product/product/detailsPage?id=' + row.id + "&iframe=" + row.id);
                /*
            ...
            submitRevoke(){
                ...
                app.ajax('/product/product/product/revokeProduct?id='+this.revokeProduct.id).then( _=> {this.get();this.revokeDialogVisible=false;});
            },
            showFSM(row){
