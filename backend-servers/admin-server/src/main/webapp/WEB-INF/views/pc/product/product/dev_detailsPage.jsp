...
<template scope='app'>
    <el-row>
        <el-col :span='7' style='padding: 15px;'>
            <el-card v-for='item in window.app.controls.products'>
                <el-row style='font-size: 12px; line-height: 24px;'>
                    ...
                    <el-col :span='24'>
                        产品ID：{{item.id}}<br> SPU: {{item.spu}}<br>内部名:{{item.innerName}}
                        ...
                        <br> 标题: {{item.title}} <br>
                        <!--标题语言包: <a href='javascript:void(0)' @click='app.controls.edit.lang = !app.controls.edit.lang'>{{app.controls.edit.lang ? '收缩' : '展开'}}</a>-->
                        ...
                        <%--<a :href='window.app.normalizeURL(item.purchaseUrl)' target="_blank">采购链接</a></span><br>--%>
                        ...
                        <el-dialog
                                ...
                                <el-button @click="window.app.controls.c.dialogVisible = false">取 消</el-button>
                                <el-button type="primary" @click="window.app.controls.c.submit(item)">确 定</el-button>
                                </span>
                        </el-dialog>
                        <br>
        ...
        <el-col :span='6' style='padding: 15px;'>
            ...
            <el-card v-for='item in window.app.controls.products'>
                <el-button type='primary' @click='app.controls.dialog.sku = true;'>查看SKU</el-button>
                ...
                <el-row style='padding: 15px 5px;  border: 2px dashed grey;'>
                    <div style='display: flex; flex-flow: row wrap;'>
                        <draggable v-model='app.controls.cateAttrList'
                            ...
                            <el-button v-for='b in app.controls.cateAttrList'
                                       :disabled="!app.controls.app.Perm.attr_write" :data-title='b.title' class='a-c'
                    ...
                    <el-col :span='24' style='padding: 15px 0px;'>
                        <div @keyup.enter='app.controls.c.addAttrValue()'>
                            <control-item type='input' :local='app.controls.edit' label='属性值名称'
                                          ...
                                          :icon='!app.controls.app.Perm.attr_write? "" : "plus"'>
                            </control-item>
                        </div>
                    </el-col>
                </el-row>
                ...
                <el-row style='padding: 15px 5px; border: 2px dashed grey;'>
                    <div style='display: flex; flex-flow: row wrap;'>
                        <draggable v-model='item'
                            ...
                            <el-button v-for='b in item' :data-id='b.id' :disabled="!app.controls.app.Perm.attr_write"
                                       :data-title='b.title' class='v-p' size='mini'
                ...
                <el-row style='padding: 15px 5px;  border: 2px dashed grey;'>
                    <div style='display: flex; flex-flow: row wrap;'>
                        <draggable v-model='item'
                            ...
                            <el-button v-for='b in item' :data-id='b.id' :disabled="!app.controls.app.Perm.attr_write"
                                       class='v-c' :data-title='b.title' size='mini'
                                       style='margin-bottom: 5px;' v-if='!b.bindIs'>
                                {{b.title}}
                            </el-button>
                        </draggable>
...
<js>
    ...
    <script>
        ...
        window.layoutData.controls = {
            ...
            dialog: {lang: false, sku: false,},
            lang: {productId: args.id, langCode: "", name: ""},
            langs: [],
            cateAttrList: [],
            attrValue: undefined,
            langPack: {
        ...
        window.layoutData.controls.p = {
            get() {
                app.ajax.product.devDetail({id: app.id}).then(x => {
            ...
            doDrag(allkind, id, title){
                ...
                switch (allkind) {
                    ...
                    case 'category.attrValue.category.attrValue': {
                        let a = _ => new Promise((ac, re) => ac(app.controls.p.get())).then(_ => app.controls.c.getAttrValue(app.controls.attrId, app.controls.attrTitle));
...
//                app.window.parent.location =window.location.origin+ '/product/product/product/dev_list';
                <%--top.location.href =  '${pageContext.request.contextPath}/product/product/product/dev_list';--%>
