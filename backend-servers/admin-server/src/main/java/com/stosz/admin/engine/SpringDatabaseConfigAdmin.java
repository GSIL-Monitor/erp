...
@Component
public class SpringDatabaseConfigAdmin extends SpringDatabaseConfigAbstract {

    @Value("${mysql.jdbc.driver}")
    private String jdbcDriver;
    @Value("${admin.jdbc.url}")
    private String jdbcUrl;
    @Value("${admin.jdbc.username}")
    private String jdbcUsername;
    @Value("${admin.jdbc.password}")
    private String jdbcPassword;

    @Value("${admin.c3p0.minPoolSize}")
    private Integer c3p0_minPoolSize;
    @Value("${admin.c3p0.maxPoolSize}")
    private Integer c3p0_maxPoolSize;
    @Value("${admin.c3p0.initialPoolSize}")
    private Integer c3p0_initialPoolSize;
    @Value("${admin.c3p0.maxIdleTime}")
    private Integer c3p0_maxIdleTime;
    @Value("${admin.c3p0.acquireIncrement}")
    private Integer c3p0_acquireIncrement;
    @Value("${admin.c3p0.idleConnectionTestPeriod}")
    private Integer c3p0_idleConnectionTestPeriod;
    @Value("${admin.c3p0.acquireRetryAttempts}")
    private Integer c3p0_acquireRetryAttempts;
    @Value("${admin.c3p0.maxStatements}")
    private Integer c3p0_maxStatements;
    @Value("${admin.c3p0.maxStatementsPerConnection}")
    private Integer c3p0_maxStatementsPerConnection;
    @Value("${admin.c3p0.checkoutTimeout}")
    private Integer c3p0_checkoutTimeout;

//    @Value("${admin.mybatis.mapperLocations}")
...
//
    ...
    public String getJdbcPassword() {
        try {
            ...
            logger.error("解密数据库连接密码时配置文件中的加密秘钥:{} 可能不正确！错误信息：{}" , secretKey, e.getMessage());
            System.exit(-1);
		}
        return null;
    }
