...
/**
 * 新加坡东丰 查询物流轨迹
 * @version [1.0,2018年1月8日]
 */
@Component
public class DfTrackHandler extends AbstractSingleTrackHandler {
	private static final Logger logger= LoggerFactory.getLogger(DfTrackHandler.class);

	public DfTrackHandler(@Qualifier("dfParseStartegy") AbstractParseStartegy parseStartegy) {
		super(parseStartegy);
	}
	...
	public TransportTrackResponse captureTransTrack(TrackRequest request, ShippingExtend shippingExtend) {
		...
		try {
			...
			paramMap.put("cno", request.getTrackingNo());
			...
			logger.trace("DfTrackHandler 物流轨迹抓取异常,trackingNo={},Exception={}", request.getTrackingNo(), e);
			trackResponse.fail("DfTrackHandler 物流轨迹抓取异常");
		}
