package com.stosz.store.erpOld.sync.core;
...
public abstract class AbstractSycn<T> {
     ...
     */
    @Transactional(value = "erpOldTransactionManager", propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public void pullById() {
        ...
        if (oldMaxId > newMaxId) {
            logger.info("开始同步 old:{},new:{},共插入{}条", oldMaxId, newMaxId, oldMaxId - newMaxId);
            //2,批量插入
